...
fn test_should_send_response() {
    ...
    rr.code = 200;
    assert_eq!(should_send_response(&globalopts, &rr, &None), false,
        "Code in blacklist failed");

    // Response code is not in blacklist -> true
    rr.code = 300;
    assert_eq!(should_send_response(&globalopts, &rr, &None), true,
    ...
    assert_eq!(should_send_response(&globalopts, &rr, &Some(val)), false,
